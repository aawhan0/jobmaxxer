...
def parse_remoteok_payload(company: str, payload: list[dict[str, Any]]) -> list[Job]:
    return [_job(company, x, 'remoteok') for x in payload if isinstance(x, dict) and x.get('position')]
...
def scan_company(company: str, url: str, adapter: str | None = None, timeout: int = 20) -> list[Job]:
    """Fetch jobs from a supported ATS or configured public source."""
